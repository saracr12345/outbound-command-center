...
function extractLinkedInUrls(text: string): {
  ...
  const matches =
    text.match(
      /https?:\/\/(?:www\.)?linkedin\.com\/(?:in|company)\/[^\s)>]+/gi,
    ) ?? [];

  const urls = matches
    .map((value) => normaliseLinkedInUrl(value))
    .filter((value): value is string => Boolean(value));

  return {
    person: urls.find((url) => /linkedin\.com\/in\//i.test(url)) ?? null,
    company:
      urls.find((url) => /linkedin\.com\/company\//i.test(url)) ?? null,
  };
}
...
export function parseRb2bSlackMessage(
  ...
  const labelledLinkedInUrl = normaliseLinkedInUrl(linkedInRaw);
  const discoveredLinkedIn = extractLinkedInUrls(rawText);

  const linkedinUrl =
    discoveredLinkedIn.person ??
    (labelledLinkedInUrl &&
    !/linkedin\.com\/company\//i.test(labelledLinkedInUrl)
      ? labelledLinkedInUrl
      : null);

  const companyLinkedinUrl =
    discoveredLinkedIn.company ??
    (labelledLinkedInUrl &&
    /linkedin\.com\/company\//i.test(labelledLinkedInUrl)
      ? labelledLinkedInUrl
      : null);
